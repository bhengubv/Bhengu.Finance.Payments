...
public sealed class MercadoPagoTokenisationProvider : ITokenisationProvider
{
    ...
    public async Task<PaymentMethod> TokeniseAsync(TokeniseRequest request, CancellationToken ct = default)
    {
        ...
        var tokenRaw = await SendAsync(HttpMethod.Post, "/v1/card_tokens", tokenBody, ct, "CreateCardToken", request.IdempotencyKey).ConfigureAwait(false);
        var token = DeserialiseOrThrow<MercadoPagoCardToken>(tokenRaw, "CreateCardToken");
        ...
        if (string.IsNullOrWhiteSpace(customerId))
        {
            ...
            var customerRaw = await SendAsync(HttpMethod.Post, "/v1/customers", customerBody, ct, "CreateCustomer", request.IdempotencyKey).ConfigureAwait(false);
            var customer = DeserialiseOrThrow<MercadoPagoCustomer>(customerRaw, "CreateCustomer");
            customerId = customer.Id;
        }

        // 3) Persist the card against the customer for a long-lived card id.
        var cardBody = new { token = token.Id };
        var cardRaw = await SendAsync(HttpMethod.Post, $"/v1/customers/{Uri.EscapeDataString(customerId ?? string.Empty)}/cards", cardBody, ct, "AttachCard", request.IdempotencyKey).ConfigureAwait(false);
        var card = DeserialiseOrThrow<MercadoPagoCard>(cardRaw, "AttachCard");

        _logger.LogInformation("Mercado Pago card vaulted: cardId={CardId} customerId={CustomerId}", card.Id, customerId);

        return MapCard(card, customerId, request.DisplayName, request.SetAsDefault);
    }
    ...
    public async Task<PaymentMethod?> GetPaymentMethodAsync(string token, CancellationToken ct = default)
    {
        ...
        try
        {
            var raw = await SendAsync(HttpMethod.Get, $"/v1/customers/cards/{Uri.EscapeDataString(token)}", body: null, ct, "GetCard", idempotencyKey: null).ConfigureAwait(false);
    ...
    public async Task<IReadOnlyList<PaymentMethod>> ListPaymentMethodsAsync(string customerId, CancellationToken ct = default)
    {
        ...
        var raw = await SendAsync(HttpMethod.Get, $"/v1/customers/{Uri.EscapeDataString(customerId)}/cards", body: null, ct, "ListCards", idempotencyKey: null).ConfigureAwait(false);

        try
        {
            var cards = JsonSerializer.Deserialize<MercadoPagoCard[]>(raw) ?? Array.Empty<MercadoPagoCard>();
            var methods = new List<PaymentMethod>(cards.Length);
            foreach (var c in cards)
                methods.Add(MapCard(c, customerId, displayName: null, isDefault: false));
            return methods;
        }
    ...
    public async Task<bool> DeletePaymentMethodAsync(string token, CancellationToken ct = default)
    {
        ...
        try
        {
            await SendAsync(HttpMethod.Delete, $"/v1/customers/{Uri.EscapeDataString(method.CustomerId)}/cards/{Uri.EscapeDataString(token)}", body: null, ct, "DeleteCard", idempotencyKey: null).ConfigureAwait(false);
            return true;
        }
    ...
    private static PaymentMethod MapCard(MercadoPagoCard card, string? customerId, string? displayName, bool isDefault)
    {
    ...
    private sealed class MercadoPagoCardToken
    {
    ...
    private sealed class MercadoPagoCustomer
    {
        [JsonPropertyName("id")] public string? Id { get; set; }
    ...
    private sealed class MercadoPagoCard
    {
        [JsonPropertyName("id")] public string? Id { get; set; }
    ...
    private sealed class MercadoPagoCardPaymentMethod
    {
        [JsonPropertyName("id")] public string? Id { get; set; }
