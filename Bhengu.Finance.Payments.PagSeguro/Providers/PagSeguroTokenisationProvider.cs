...
public sealed class PagSeguroTokenisationProvider : BhenguProviderBase, ITokenisationProvider
{
    ...
    /// <inheritdoc />
#pragma warning disable CS1998 // intentionally async with no awaits — PagBank exposes no list endpoint, but the contract demands IAsyncEnumerable.
...
#pragma warning restore CS1998
    ...
    internal static PaymentMethod MapToken(PagSeguroToken t, string? customerId, string? displayName, bool isDefault)
    {
    ...
    // === PagSeguro API response shapes (internal, shared with raw-card variant) ===

    internal sealed class PagSeguroToken
    {
    ...
    internal sealed class PagSeguroTokenCard
    {
...
/// <summary>Shared HTTP helper for PagSeguro vault operations.</summary>
internal static class PagSeguroHttp
{
    public static async Task<string> SendAsync(HttpClient httpClient, ILogger logger, string providerName, HttpMethod method, string path, object? body, CancellationToken ct, string operation)
    {
        ...
        if (body is not null)
        {
            var json = JsonSerializer.Serialize(body, PagSeguroTokenisationProvider.WriteOptions);
