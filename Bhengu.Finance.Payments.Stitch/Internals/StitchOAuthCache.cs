...
namespace Bhengu.Finance.Payments.Stitch.Internals;

/// <summary>
/// Stitch OAuth2 client_credentials token cache backed by <see cref="IBhenguDistributedCache"/>.
/// Used by <c>StitchMandateProvider</c> (DebiCheck requires a scoped bearer token; the simpler
/// <c>X-API-Key</c> path that <c>StitchPaymentProvider</c> uses does not). With a Redis
/// implementation every replica shares the same access token until <c>expires_in − 60s</c>,
/// so Stitch's <c>/connect/token</c> is hit at most once per token lifetime per client id —
...
/// <see cref="ConcurrentDictionary{TKey, TValue}"/>; only the first caller hits Stitch, peers
...
/// </remarks>
public sealed class StitchOAuthCache
{
    private const string KeyPrefix = "stitch:oauth:";
    ...
    public StitchOAuthCache(IBhenguDistributedCache cache)
    {
    ...
    /// </summary>
    public StitchOAuthCache() : this(new InMemoryBhenguDistributedCache()) { }

    /// <summary>
    /// Return a valid Stitch access token for <paramref name="clientId"/>. Cache hits return
    ...
    /// </summary>
    /// <param name="clientId">Stitch OAuth2 client id — used to derive the cache slot.</param>
    /// <param name="fetch">Async factory performing the actual <c>/connect/token</c> POST and
