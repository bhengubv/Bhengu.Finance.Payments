...
/// <summary>OTel counter assertions for the MTNMoMo provider family.</summary>
public class MTNMoMoDiagnosticsTests
{
    [Fact]
...
""".Trim()));
        var provider = new MTNMoMoPaymentProvider(
            new HttpClient(handler),
            Options.Create(new MTNMoMoOptions { SubscriptionKey = "sk", ApiUserId = "u", ApiKey = "k", TargetEnvironment = "sandbox" }),
        ...
        Assert.True(recorder.CounterTotalFor("bhengu_payments_charges_total", "mtnmomo") >= 1);
    }
}