...
/// <summary>OTel counter assertions for the PagSeguro provider family.</summary>
public class PagSeguroDiagnosticsTests
{
    [Fact]
...
{"id":"pag-diag","status":"PAID","charges":[{"status":"PAID"}]}
""".Trim()));
        var provider = new PagSeguroPaymentProvider(
            new HttpClient(handler),
            Options.Create(new PagSeguroOptions { ApiToken = "tok" }),
        ...
        Assert.True(recorder.CounterTotalFor("bhengu_payments_charges_total", "pagseguro") >= 1);
    }
}