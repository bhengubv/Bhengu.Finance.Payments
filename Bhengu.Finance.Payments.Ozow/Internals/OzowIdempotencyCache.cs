...
namespace Bhengu.Finance.Payments.Ozow.Internals;

/// <summary>
/// Client-side dedup wrapper for Ozow. Ozow's API does not currently expose a native
/// Idempotency-Key header — this cache memoises the SDK-side response so a retry that reaches
/// the SDK never re-issues an HTTP call to Ozow.
/// </summary>
public sealed class OzowIdempotencyCache
{
    ...
    public OzowIdempotencyCache(IBhenguDistributedCache cache, TimeSpan? ttl = null)
    {
    ...
    public async Task<T> GetOrAddAsync<T>(string? idempotencyKey, Func<Task<T>> factory, CancellationToken ct = default) where T : class
    {
        ...
        var key = $"ozow:idem:{idempotencyKey}";
