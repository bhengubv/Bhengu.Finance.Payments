...
public class PayFastIntegrationTests
{
    private readonly IPayFastService _service;

    public PayFastIntegrationTests()
    {
        var config = new ConfigurationBuilder()
            .AddJsonFile("appsettings.Development.json")
            .Build();
        var loggerFactory = LoggerFactory.Create(builder => builder.AddConsole());
        var logger = loggerFactory.CreateLogger<PayFastService>();
        var client = new HttpClient();
        _service = new PayFastService(client, config, logger);
    }
