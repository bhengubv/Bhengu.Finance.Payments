...
/// <summary>DRY OpenTelemetry observation helpers for the TymeBank provider family.</summary>
internal static class TymeBankObservability
{
    public static async Task<T> ObserveChargeAsync<T>(string currency, Func<Task<T>> body)
    {
        using var activity = BhenguPaymentDiagnostics.StartChargeActivity(ProviderNames.TymeBank, currency);
        ...
        finally
        {
            ...
            BhenguPaymentDiagnostics.ChargesTotal.Add(1,
                new KeyValuePair<string, object?>("provider", ProviderNames.TymeBank),
    ...
    {
        using var activity = BhenguPaymentDiagnostics.StartRefundActivity(ProviderNames.TymeBank, gatewayReference);
        var outcome = BhenguPaymentDiagnostics.Outcomes.Success;
        try { return await body().ConfigureAwait(false); }
        catch (PaymentDeclinedException) { outcome = BhenguPaymentDiagnostics.Outcomes.Declined; throw; }
        catch (ProviderRateLimitException) { outcome = BhenguPaymentDiagnostics.Outcomes.RateLimited; throw; }
    ...
    {
        using var activity = BhenguPaymentDiagnostics.StartPayoutActivity(ProviderNames.TymeBank, currency);
        var outcome = BhenguPaymentDiagnostics.Outcomes.Success;
        try { return await body().ConfigureAwait(false); }
        catch (PaymentDeclinedException) { outcome = BhenguPaymentDiagnostics.Outcomes.Declined; throw; }
        catch (ProviderRateLimitException) { outcome = BhenguPaymentDiagnostics.Outcomes.RateLimited; throw; }
    ...
    {
        using var activity = BhenguPaymentDiagnostics.StartOperationActivity(ProviderNames.TymeBank, operationName);
        var outcome = BhenguPaymentDiagnostics.Outcomes.Success;
        try { return await body().ConfigureAwait(false); }
        catch (PaymentDeclinedException) { outcome = BhenguPaymentDiagnostics.Outcomes.Declined; throw; }
        catch (ProviderRateLimitException) { outcome = BhenguPaymentDiagnostics.Outcomes.RateLimited; throw; }
    ...
    {
        using var activity = BhenguPaymentDiagnostics.StartWebhookActivity(ProviderNames.TymeBank);
        var outcome = BhenguPaymentDiagnostics.Outcomes.Success;
        try { return await body().ConfigureAwait(false); }
        catch { outcome = BhenguPaymentDiagnostics.Outcomes.Error; throw; }
        finally { activity.SetOutcome(outcome); }
